...
document.querySelector('#url').onkeydown=async e=>{if(e.key==='Enter'){const url=e.target.value.trim();if(!/^https?:\/\//i.test(url)) return;pendingUrls.push(url);e.target.value='';document.querySelector('#queued').textContent=pendingUrls.length;try{await window.zelux?.startDownload?.([url]);}catch(err){document.querySelector('.ready').textContent='● Error';}}};
